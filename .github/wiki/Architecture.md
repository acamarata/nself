...
These four services are always present. Every nSelf project runs them.

### PostgreSQL (`postgres:16-alpine`, port 5432)

The primary database for all project data. nSelf initialises it with three extensions enabled by default: `uuid-ossp` for UUID generation, `pgcrypto` for password hashing and encryption primitives, and `pg_trgm` for trigram-based full-text search. Hasura and Auth both connect to this instance — there is one Postgres, not two.
...
nSelf auto-generates the Nginx configuration from your project settings. You do not write `nginx.conf` by hand.
...
This also means you can run nSelf on a server with a minimal firewall: open ports 80 and 443, close everything else.
...
nSelf reserves ten slots for user-defined services: `CS_1` through `CS_10`. Each slot takes a single string in the format `name:template:port:route`. The CLI reads these values, pulls the matching service template, and injects it into the generated compose file alongside your core services.
