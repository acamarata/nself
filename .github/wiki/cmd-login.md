...
`nself login` authenticates the CLI with your nSelf account (cloud.nself.org or a self-hosted nSelf Cloud instance). On success, a session token is stored in the local credential store (`~/.config/nself/credentials.json`) and used automatically by subsequent commands that require authentication.
