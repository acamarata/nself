...
nSelf generates required secrets and repairs common misconfigurations automatically on first boot and on every `nself build` / `nself deploy`. No manual secret management is required for baseline operation.
...
nSelf now generates the following in every nginx server block for upstream proxying:
...
This means nSelf's AI features work on a fresh install without any API keys or cloud accounts. Users on slower hardware can override with `OLLAMA_MODEL` env var.
