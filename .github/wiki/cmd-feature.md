...
> Moved. This command now lives at `nself config features`.

<!-- Redirect stub. GitHub Wiki has no server-side redirects, so this page stays
     in place for bookmarks and external links. -->

`nself config features` is the current spelling. The old top-level spelling still works and
...
**Read the current documentation here: [[cmd-config]]**
