# nself Guides

Comprehensive guides for using nself effectively.

---

## Getting Started Guides

Essential guides for new users:

| Guide | Description |
|-------|-------------|
| **[Quick Start](../getting-started/Quick-Start.md)** | Get running in 5 minutes |
| **[Installation](../getting-started/Installation.md)** | Detailed installation guide |
| **[Database Workflow](DATABASE-WORKFLOW.md)** | DBML to production in one command |
| **[FAQ](../getting-started/FAQ.md)** | Frequently asked questions |

---

## Core Workflows

Master the essential workflows:

### Database & Development
- **[Database Workflow](DATABASE-WORKFLOW.md)** - Design-first database development with DBML
- **[Examples](EXAMPLES.md)** - Real-world code examples and patterns
- **[Multi-App Setup](MULTI_APP_SETUP.md)** - Running multiple apps in one stack
- **[Service Communication](SERVICE-TO-SERVICE-COMMUNICATION.md)** - Microservices patterns

### Deployment & Operations
- **[Deployment](Deployment.md)** - Production deployment with SSH and zero-downtime
- **[Deployment Architecture](DEPLOYMENT-ARCHITECTURE.md)** - Understanding deployment patterns
- **[Environments](ENVIRONMENTS.md)** - Managing local, staging, and production
- **[Backup Guide](BACKUP_GUIDE.md)** - Database backup and recovery

### Security & Production
- **[Security](SECURITY.md)** - Production security best practices
- **[SSL/TLS Setup](../configuration/SSL.md)** - Certificate management
- **[Secrets Management](../configuration/SECRETS-MANAGEMENT.md)** - Handling sensitive data

---

## Feature Guides

Guides for specific features:

### Authentication & Users
- **[OAuth Setup](OAUTH-SETUP.md)** - Social login configuration (Google, GitHub, etc.)
- **[OAuth Quick Start](OAUTH-QUICK-START.md)** - Fast OAuth setup
- **[Organization Management](ORGANIZATION-MANAGEMENT.md)** - Multi-tenancy and team management

### Advanced Features
- **[Real-Time Features](REALTIME-FEATURES.md)** - GraphQL subscriptions and live data
- **[Billing & Usage](BILLING-AND-USAGE.md)** - Usage tracking and billing integration
- **[Plugin Development](PLUGIN-DEVELOPMENT.md)** - Creating custom plugins

---

## Troubleshooting

When things go wrong:

- **[Troubleshooting](TROUBLESHOOTING.md)** - Common issues and solutions
- **[Doctor Command](../commands/DOCTOR.md)** - Automated diagnostics
- **[Billing Troubleshooting](../troubleshooting/BILLING-TROUBLESHOOTING.md)** - Billing-specific issues
- **[White-Label Troubleshooting](../troubleshooting/WHITE-LABEL-TROUBLESHOOTING.md)** - Branding issues

---

## By Topic

### Development
- [Quick Start](../getting-started/Quick-Start.md)
- [Database Workflow](DATABASE-WORKFLOW.md)
- [Service Communication](SERVICE-TO-SERVICE-COMMUNICATION.md)
- [Examples](EXAMPLES.md)

### Configuration
- [Environments](ENVIRONMENTS.md)
- [Multi-App Setup](MULTI_APP_SETUP.md)
- [OAuth Setup](OAUTH-SETUP.md)

### Deployment
- [Deployment](Deployment.md)
- [Deployment Architecture](DEPLOYMENT-ARCHITECTURE.md)
- [Backup Guide](BACKUP_GUIDE.md)

### Advanced
- [Real-Time Features](REALTIME-FEATURES.md)
- [Organization Management](ORGANIZATION-MANAGEMENT.md)
- [Billing & Usage](BILLING-AND-USAGE.md)
- [Plugin Development](PLUGIN-DEVELOPMENT.md)

### Security
- [Security](SECURITY.md)
- [SSL/TLS Setup](../configuration/SSL.md)
- [Secrets Management](../configuration/SECRETS-MANAGEMENT.md)

---

## Learning Paths

### New to nself?
1. [Quick Start](../getting-started/Quick-Start.md)
2. [Installation](../getting-started/Installation.md)
3. [Database Workflow](DATABASE-WORKFLOW.md)
4. [FAQ](../getting-started/FAQ.md)

### Building an Application?
1. [Database Workflow](DATABASE-WORKFLOW.md)
2. [Service Communication](SERVICE-TO-SERVICE-COMMUNICATION.md)
3. [OAuth Setup](OAUTH-SETUP.md)
4. [Real-Time Features](REALTIME-FEATURES.md)

### Going to Production?
1. [Deployment](Deployment.md)
2. [Environments](ENVIRONMENTS.md)
3. [Security](SECURITY.md)
4. [Backup Guide](BACKUP_GUIDE.md)

### Building a SaaS?
1. [Organization Management](ORGANIZATION-MANAGEMENT.md)
2. [Billing & Usage](BILLING-AND-USAGE.md)
3. [Multi-App Setup](MULTI_APP_SETUP.md)
4. [Deployment Architecture](DEPLOYMENT-ARCHITECTURE.md)

---

## Related Documentation

- **[Commands](../commands/README.md)** - CLI command reference
- **[Configuration](../configuration/README.md)** - Configuration options
- **[Services](../services/SERVICES.md)** - Available services
- **[Architecture](../architecture/ARCHITECTURE.md)** - System architecture
- **[Tutorials](../tutorials/README.md)** - Step-by-step tutorials

---

**[Back to Documentation Home](../README.md)**
