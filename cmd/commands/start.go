...
func runStart(cmd *cobra.Command, _ []string) error {
	...
	if err := checkDockerAvailable(cmd.Context()); err != nil {
		return err
	}
	...
	checkLicenseHeartbeat(cmd.Context(), cfg, opts.verbose)
	...
	if opts.cleanStart {
		...
		sp.Start()
		if err := compose.ComposeDown(cmd.Context(), projectDir, docker.DownOptions{
			RemoveOrphans: true,
		}); err != nil {
	...
	if opts.fresh && !opts.cleanStart {
		...
		sp.Start()
		if err := compose.ComposeDown(cmd.Context(), projectDir, docker.DownOptions{}); err != nil {
	...
	sp.Start()
	if err := compose.ComposeUp(cmd.Context(), projectDir, "postgres"); err != nil {
	...
	dbCtx, dbCancel := context.WithTimeout(cmd.Context(), 90*time.Second)
	defer dbCancel()
	...
	svcSp.Start()
	if err := compose.ComposeUp(cmd.Context(), projectDir); err != nil {
	...
	if opts.skipHealthChecks {
		...
		report := runHealthCheckLoop(cmd.Context(), cfg, opts.timeout, requiredPct, opts.verbose)
