...
// Purpose: nself encryption rotate/status/key-events subcommands, BYOK HTTP helpers, and the group's cobra flag/AddCommand wiring, split out of encryption.go's root/configure/verify commands.
// Inputs: cobra flags for rotate (dry-run) and the BYOK service base URL/key ref resolved from environment.
// Outputs: formatted rotation/status/key-event reports via the BYOK API.
// Constraints: split out of encryption.go as a pure move (CLI-R12); no behavior change.
