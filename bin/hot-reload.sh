...
detect_changes() {
    ...
    if [ ! -f "$LAST_ENV_FILE" ]; then
        echo_info "No previous build state found. Full build required."
        return 1
    fi
    ...
    if [ "$current_checksum" == "$last_checksum" ]; then
        echo_success "No configuration changes detected."
        return 0
    fi
    
    echo_info "Configuration changes detected. Analyzing..."
        ...
        for prefix in "APP_${i}_ROUTE" "APP_ROUTE_${i}"; do
            ...
            if [ "$current_val" != "$last_val" ]; then
                if [ -n "$current_val" ] && [ -z "$last_val" ]; then
                    echo_info "  + New app route: $prefix=$current_val"
                    needs_nginx_reload=true
                elif [ -z "$current_val" ] && [ -n "$last_val" ]; then
                    echo_info "  - Removed app route: $prefix"
                    needs_nginx_reload=true
                elif [ -n "$current_val" ] && [ -n "$last_val" ]; then
                    echo_info "  ~ Modified app route: $prefix"
                    needs_nginx_reload=true
                fi
            fi
        done
    ...
    for service_type in "GOLANG_SERVICES" "PYTHON_SERVICES" "NESTJS_SERVICES" "BULLMQ_WORKERS"; do
        ...
        if [ "$current_services" != "$last_services" ]; then
            echo_info "  ~ Changes in $service_type"
            ...
            for service in "${last_arr[@]}"; do
                ...
                if [[ ! " ${current_arr[@]} " =~ " ${service} " ]]; then
                    echo_info "    - Removed service: $service"
    ...
    if [ "$current_ext" != "$last_ext" ]; then
        echo_info "  ~ PostgreSQL extensions changed"
    ...
    for var in "POSTGRES_VERSION" "HASURA_VERSION" "NGINX_VERSION"; do
        ...
        if [ "$current_ver" != "$last_ver" ] && [ -n "$current_ver" ] && [ -n "$last_ver" ]; then
            echo_info "  ~ $var changed from $last_ver to $current_ver"
            needs_full_rebuild=true
        fi
    done
    ...
    if [ "$needs_full_rebuild" = true ]; then
        echo_warning "Full rebuild required due to major changes."
        return 2
...
apply_changes() {
    ...
    if [ "$dry_run" = true ]; then
        echo_info "DRY RUN: Showing what would be changed..."
    fi
        ...
        if [ "$dry_run" = false ]; then
            ...
            for i in {0..20}; do
                ...
                if [[ -n "$route_value" ]]; then
                    ...
                    echo_info "  Configuring route: $subdomain (localhost:$port -> $domain)"
                    ...
                    if [ "$dry_run" = false ]; then
                        ...
                        echo_info "  Removed config for app route $i"
                    else
                        echo_info "  Would remove config for app route $i"
                    fi
                fi
            done
            ...
            if [ "$dry_run" = false ]; then
                ...
                docker exec ${PROJECT_NAME}_nginx nginx -s reload || {
                    echo_warning "Failed to reload nginx. Container might not be running."
                }
            else
                echo_info "Would reload nginx"
            fi
        fi
    fi
    ...
    if [ -f "$STATE_DIR/.new_services" ]; then
        while IFS= read -r service_entry; do
            if [ -n "$service_entry" ]; then
                ...
                echo_info "Adding new service: $service_name ($service_type)"
                ...
                if [ "$dry_run" = false ]; then
                    ...
                    echo_info "  Would generate and start $service_name"
                fi
            fi
    ...
    if [ -f "$STATE_DIR/.removed_services" ]; then
        while IFS= read -r service_entry; do
            if [ -n "$service_entry" ]; then
                IFS=':' read -r service_type service_name <<< "$service_entry"
                echo_info "Removing service: $service_name"
                ...
                if [ "$dry_run" = false ]; then
                    ...
                    echo_info "  Service $service_name stopped (code preserved)"
                else
                    echo_info "  Would stop and remove $service_name container"
                fi
            fi
    ...
    echo_success "Changes applied successfully!"
}
...
hot_reload() {
    ...
    case "$action" in
        check)
            ...
            if [ $status -eq 0 ]; then
                echo_success "No changes to apply."
            elif [ $status -eq 2 ]; then
                echo_warning "Full rebuild required. Run 'nself build' to apply changes."
            elif [ $status -eq 3 ]; then
                echo_info "Incremental changes can be applied."
                echo_info "Run 'nself up --apply-changes' to apply them."
            fi
            ;;
        ...
        save-state)
            ...
            echo_success "Current state saved."
            ;;
            
        *)
            echo_error "Unknown action: $action"
            echo_info "Valid actions: check, apply, dry-run, save-state"
            exit 1
            ;;
    esac
}
