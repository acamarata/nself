...
ssl::init() {
  ...
  # Generate nginx location block for ACME challenge serving
  ssl::generate_acme_nginx_config

  log_success "SSL directory initialized: $SSL_DIR"
}

# Generate nginx config snippet for serving ACME challenge responses.
# This location block allows Let's Encrypt / acme.sh to validate domain
# ownership via HTTP-01 challenge while the rest of port 80 redirects
# to HTTPS.
ssl::generate_acme_nginx_config() {
  local acme_conf="${NGINX_CONF_DIR:-nginx/conf.d}/acme-challenge.conf"
  local webroot="$CERTBOT_WEBROOT"

  mkdir -p "$(dirname "$acme_conf")" 2>/dev/null || true

  cat >"$acme_conf" <<EOF
# ACME challenge location for Let's Encrypt / acme.sh
# Generated by nself ssl init
# This server block handles HTTP-01 challenges on port 80.
server {
    listen 80;
    listen [::]:80;
    server_name _;
...
EOF
  ...
  log_info "ACME challenge nginx config written to $acme_conf"
  return 0
}
...
ssl::check_acme_sh() {
  ...
  local acme_paths="$HOME/.acme.sh/acme.sh /usr/local/bin/acme.sh"
...
# Get the path to the ACME client (certbot preferred, acme.sh fallback)
ssl::get_acme_client() {
  ...
  if ssl::check_acme_sh; then
    # Return the full path to acme.sh
    if command -v acme.sh >/dev/null 2>&1; then
      printf "acme.sh"
    elif [[ -x "$HOME/.acme.sh/acme.sh" ]]; then
      printf "%s/.acme.sh/acme.sh" "$HOME"
    elif [[ -x "/usr/local/bin/acme.sh" ]]; then
      printf "/usr/local/bin/acme.sh"
    fi
    return 0
  fi
...
ssl::request_cert() {
  ...
  # Determine which ACME client to use (certbot preferred, acme.sh fallback)
  local acme_client
  acme_client=$(ssl::get_acme_client 2>/dev/null || true)

  if [[ -z "$acme_client" ]]; then
    log_warning "Neither certbot nor acme.sh is installed"
    ssl::install_certbot
    return 1
  fi

  log_info "Requesting SSL certificate for: $domain (using $acme_client)"

  if [[ "$acme_client" == "certbot" ]]; then
    ssl::_request_via_certbot "$domain" "$email" "$method" "$staging"
  else
    ssl::_request_via_acme_sh "$acme_client" "$domain" "$email" "$method" "$staging"
  fi
}

# Internal: request certificate via certbot
ssl::_request_via_certbot() {
  local domain="$1"
  local email="$2"
  local method="$3"
  local staging="$4"
...
# Internal: request certificate via acme.sh (fallback when certbot unavailable)
ssl::_request_via_acme_sh() {
  local acme_sh="$1"
  local domain="$2"
  local email="$3"
  local method="$4"
  ...
  case "$method" in
    webroot)
      ssl::init
      acme_cmd="$acme_cmd -w $CERTBOT_WEBROOT"
      ;;
    standalone)
      ...
      ;;
    dns)
      acme_cmd="$acme_cmd --dns"
      log_warning "DNS challenge requires manual DNS record creation or DNS API plugin"
      ;;
    *)
      log_error "Unknown method: $method (use webroot, standalone, or dns)"
      return 1
  ...
  if bash -c "$acme_cmd"; then
    log_success "Certificate obtained via acme.sh"

    # Install certificate to our SSL directory
    local install_cmd="$acme_sh --install-cert -d $domain"
    install_cmd="$install_cmd --cert-file $SSL_DIR/cert.pem"
    install_cmd="$install_cmd --key-file $SSL_DIR/key.pem"
    install_cmd="$install_cmd --fullchain-file $SSL_DIR/fullchain.pem"

    if bash -c "$install_cmd"; then
      chmod 644 "$SSL_DIR/cert.pem" "$SSL_DIR/fullchain.pem" 2>/dev/null || true
      chmod 600 "$SSL_DIR/key.pem"
      log_success "Certificate installed to: $SSL_DIR"
    else
      log_error "Failed to install certificate files"
      return 1
    fi
    return 0
  else
    log_error "Failed to obtain certificate via acme.sh"
    return 1
  fi
}
...
# Export functions
export -f ssl::init
export -f ssl::generate_acme_nginx_config
export -f ssl::check_certbot
export -f ssl::check_acme_sh
export -f ssl::get_acme_client
