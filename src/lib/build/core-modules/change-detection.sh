...
save_build_state() {
...

  {
    ...
    echo "postgres_enabled=$POSTGRES_ENABLED"
    echo "hasura_enabled=$HASURA_ENABLED"
    ...
    echo "nself_admin_enabled=$NSELF_ADMIN_ENABLED"
    echo "meilisearch_enabled=$MEILISEARCH_ENABLED"
    echo "mailpit_enabled=$MAILPIT_ENABLED"
    echo "mlflow_enabled=$MLFLOW_ENABLED"
    echo "functions_enabled=$FUNCTIONS_ENABLED"
    echo "monitoring_enabled=$MONITORING_ENABLED"
    echo "custom_service_count=$CUSTOM_SERVICE_COUNT"
    echo "frontend_app_count=$FRONTEND_APP_COUNT"
  } > "$state_file"
}
