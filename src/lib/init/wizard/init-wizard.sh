...
run_config_wizard() {
  ...
  # Database Name
  local db_name
  local default_db="mydb"
  if [[ "$project_name" != "myproject" ]]; then
    default_db="${project_name//-/_}_db"
  fi
  prompt_input "Database name" "$default_db" db_name "^[a-z][a-z0-9_]*$"
  ...
  # Check if any extensions were selected (safely handle empty array)
  local num_selected=0
  if [[ -n "${selected_extensions+x}" ]]; then
    num_selected="${#selected_extensions[@]}"
  fi
  ...
  if [[ $num_selected -gt 0 ]]; then
    ...
    # Default extensions when none selected
  ...
  local jwt_key
  if [[ "$env_mode" == "dev" ]]; then
    jwt_key="dev-jwt-secret-key-minimum-32-characters-long"
    log_info "Using development JWT key: dev-jwt-secret-key..."
  else
  ...
  echo "  ✅ Hasura - GraphQL API"
  echo "  ✅ Auth - Authentication"
  echo "  ✅ Nginx - Reverse proxy"
  echo ""

  echo "Optional Services:"
  if [[ " ${config[@]} " =~ "STORAGE_ENABLED=true" ]]; then
    echo "  ✅ Storage (MinIO)"
  fi
  if [[ " ${config[@]} " =~ "DASHBOARD_ENABLED=true" ]] || [[ " ${config[@]} " =~ "NSELF_ADMIN_ENABLED=true" ]]; then
    echo "  ✅ Admin Dashboard"
  fi
  ...
  if [[ " ${config[@]} " =~ "SEARCH_ENABLED=true" ]]; then
    local search_engine=""
    if [[ " ${config[@]} " =~ "meilisearch" ]]; then
      search_engine="MeiliSearch"
    elif [[ " ${config[@]} " =~ "elasticsearch" ]]; then
      search_engine="Elasticsearch"
    elif [[ " ${config[@]} " =~ "typesense" ]]; then
      search_engine="Typesense"
    fi
    echo "  ✅ Search ($search_engine)"
  fi
  ...
  if [[ "$env_mode" == "dev" ]]; then
    ...
    echo "  📝 Simple passwords for easy development"
...
EOF
  ...
  echo "📁 Files Created:"
  if [[ "$env_mode" == "dev" ]]; then
    echo "   .env.dev    → Team-shared development settings"
    echo "   .env        → Personal local overrides (optional)"
  else
  ...
  if [[ " ${config[@]} " =~ "USE_PORTS_ONLY=true" ]]; then
    ...
    if [[ " ${config[@]} " =~ "DASHBOARD_ENABLED=true" ]] || [[ " ${config[@]} " =~ "NSELF_ADMIN_ENABLED=true" ]]; then
      echo "      🎛️  Admin:   http://localhost:3005"
    fi
    ...
    if [[ " ${config[@]} " =~ "DASHBOARD_ENABLED=true" ]] || [[ " ${config[@]} " =~ "NSELF_ADMIN_ENABLED=true" ]]; then
      echo "      🎛️  Admin:   https://admin.localhost"
    fi
  else
    ...
    if [[ " ${config[@]} " =~ "DASHBOARD_ENABLED=true" ]] || [[ " ${config[@]} " =~ "NSELF_ADMIN_ENABLED=true" ]]; then
      echo "      🎛️  Admin:   https://admin.$base_domain"
    fi
  fi
  echo ""
  ...
  if [[ "$env_mode" == "dev" ]]; then
    ...
    echo ""
    echo "   Hasura:"
    echo "      Admin Secret: $hasura_secret"
    echo ""
    echo "   (Simple passwords for easy development)"
  else
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ...
    echo ""
    echo "   These are saved in .env but should be stored in a password manager."
  fi
  echo ""
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
}
