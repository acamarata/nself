...
provider_hostwinds_init() {
  ...
  local config_dir="${HOME}/.nself/providers"; mkdir -p "$config_dir"
...
provider_hostwinds_validate() {
  ...
  [[ -f "$config_file" ]] && { log_success "Hostwinds configured"; return 0; }
  log_error "Hostwinds not configured"; return 1
}
...
provider_hostwinds_provision() { log_warning "Hostwinds provisioning via portal: https://www.hostwinds.com"; return 1; }
provider_hostwinds_destroy() { log_warning "Manage via Hostwinds portal"; return 1; }
...
provider_hostwinds_ssh() { local ip="$1"; shift; ssh "root@$ip" "$@"; }
