#!/usr/bin/env bash
# down.sh - Stop all services with enhanced feedback
...
cmd_down() {
    ...
    if [[ -n "$services_to_stop" ]]; then
        show_command_header "nself down" "Stop services and containers"
    ...
    if [[ $running_count -eq 0 ]]; then
        ...
        if [[ -n "$stopped_containers" ]]; then
            ...
            if [[ "$remove_volumes" == true ]] || [[ "$remove_images" == true ]]; then
                ...
                echo -e "   Run ${COLOR_BLUE}nself down --volumes${COLOR_RESET} to remove all data"
    ...
    if [[ $result -eq 0 ]]; then
        ...
        # Show next steps
        echo
        echo "nself up | nself status | nself clean"
    else
...
# Show help
show_down_help() {
    echo "Usage: nself down [OPTIONS] [SERVICES...]"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  nself down                    # Stop all services, keep data"
    echo "  nself down postgres           # Stop only postgres"
    echo "  nself down --volumes          # Stop and remove all data"
    echo "  nself down --rmi              # Stop and remove images"
    echo "  nself down --volumes --rmi    # Full cleanup"
    echo ""
