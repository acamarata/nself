#!/usr/bin/env bash
# cloud.sh - Unified cloud infrastructure management
...
EXAMPLES:
  nself cloud provider list              # List all 26 providers
  nself cloud provider init digitalocean # Configure DigitalOcean
  nself cloud server create --provider hetzner --size medium
  nself cloud server list                # List all your servers
  nself cloud server ssh myserver        # SSH into server

  nself cloud cost estimate --size medium
  nself cloud cost compare --cpu 2 --ram 4
...
See 'nself cloud <subcommand> --help' for subcommand-specific options.
EOF
}
...
# Managed servers across all cloud providers
servers: []
EOF
  fi
}
...
cmd_cloud_provider() {
  ...
  case "$action" in
    ...
    *)
      ...
      log_info "Available: list, init, validate, info"
      return 1
      ;;
  esac
}
...
cmd_provider_list() {
  ...
  printf "Use 'nself cloud provider init <provider>' to configure\n\n"
}
...
cmd_provider_init() {
  ...
  if [[ -z "$provider" ]]; then
    ...
    log_info "Usage: nself cloud provider init <provider>"
    log_info "Use 'nself cloud provider list' to see available providers"
    return 1
  fi
...
cmd_server_create() {
  ...
  if [[ -z "$provider" ]]; then
    ...
    log_info "Example: nself cloud server create --provider digitalocean --size medium"
    return 1
  fi
  ...
  if type "provider_${provider}_validate" &>/dev/null; then
    if ! "provider_${provider}_validate" 2>/dev/null; then
      log_error "Provider $provider not configured. Run: nself cloud provider init $provider"
      return 1
    fi
  fi
  ...
  if type "provider_${provider}_provision" &>/dev/null; then
    ...
    if [[ -n "$server_id" ]]; then
      ...
      if [[ -n "$ip" ]]; then
        ...
        log_info "Check status: nself cloud server status $name"
      fi
    fi
  else
...
cmd_server_add() {
  ...
  if [[ -z "$ip" ]]; then
    ...
    log_info "Usage: nself cloud server add <ip> --name <name> --provider <provider>"
    return 1
  fi
...
main() {
  ...
  case "$subcommand" in
    ...
    # Shortcuts
    init)
      cmd_cloud_provider init "$@"
      ;;
    create)
      cmd_cloud_server create "$@"
      ;;
    destroy)
      cmd_cloud_server destroy "$@"
      ;;
    list|ls)
      cmd_cloud_server list "$@"
      ;;
    ssh)
      cmd_cloud_server ssh "$@"
      ;;
    *)
